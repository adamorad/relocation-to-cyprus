...
const FoodPanel = dynamic(() => import("./FoodPanel"), {
	ssr: false,
	loading: () => null,
});
const HotelsPanel = dynamic(() => import("./HotelsPanel"), {
	ssr: false,
	loading: () => null,
});
const ShoppingPanel = dynamic(() => import("./ShoppingPanel"), {
	ssr: false,
	loading: () => null,
});
const SchoolsPanel = dynamic(() => import("./SchoolsPanel"), {
	ssr: false,
	loading: () => null,
});
const HealthcarePanel = dynamic(() => import("./HealthcarePanel"), {
	ssr: false,
	loading: () => null,
});
...
const SECTIONS: ReadonlyArray<Section> = [
	...
	{
		name: "Food",
		icon: <UtensilsCrossed className={ICON_CLS} />,
		kind: "available",
	},
	{
		name: "Shopping",
		icon: <ShoppingBag className={ICON_CLS} />,
		kind: "available",
	},
	{
		name: "Schools",
		icon: <GraduationCap className={ICON_CLS} />,
		kind: "available",
	},
	{
		name: "Healthcare",
	...
	{
		name: "More",
		icon: <MoreHorizontal className={ICON_CLS} />,
		kind: "link",
		href: "/explore/",
	},
];
...
function MobileDrawer({
	...
	return (
		/* Category drawer — opens from the map "Categories" button on all sizes */
		<div
					...
					{SECTIONS.map((s) => {
						...
						const rowContent = (
							<div className="flex items-center gap-3 px-5 py-3.5">
								<span
									className="w-7 flex items-center justify-center text-slate-600"
									aria-hidden
								>
									{s.icon}
								</span>
...
export default function AppShell() {
	const nav = useMapNav()!;
	const [mapMode, setMapMode] = useState<"illustrated" | "google">(
		"illustrated",
	);
        ...
        correct image pixel regardless of viewport. */}
				<div
					className={
						mapMode === "google"
							? "absolute inset-0"
							: "relative w-full aspect-[1672/941] md:absolute md:inset-0 md:m-auto md:max-h-screen md:max-w-[177.78vh]"
					}
				>
					{selectedRegion === null && (
				...
				<FoodPanel open={nav.foodOpen} onClose={nav.closeFood} />

				<ShoppingPanel open={nav.shoppingOpen} onClose={nav.closeShopping} />

				<HotelsPanel open={nav.hotelsOpen} onClose={nav.closeHotels} />

				<SchoolsPanel open={nav.schoolsOpen} onClose={nav.closeSchools} />
